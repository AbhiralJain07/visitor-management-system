...
import { Plus, Edit2, Trash2, X, Tags, ListCollapse, ArrowUp, ArrowDown, Settings, Check, AlertTriangle } from 'lucide-react';

export const MasterDataPage: React.FC = () => {
  const { masterData, createItem, updateItem, deleteItem } = useCustomMasterData();
  ...
  const onSubmit = async (data: MasterDataFormValues) => {
    try {
      ...
      console.error('Master data form submission failed:', err);
    }
  };
              ...
              items.map((item) => (
                <div key={item._id} className="p-4 space-y-3 hover:bg-slate-50/60 transition-colors">
                  ...
                  <div className="flex gap-2 pt-2.5 border-t border-slate-100">
                    ...
                    <button
                      onClick={() => handleDelete(item._id)}
                      className="flex-1 min-h-[44px] flex items-center justify-center gap-1 px-3 py-2 bg-red-50 border border-red-200 hover:bg-red-100 rounded-xl text-red-750 font-bold text-xs transition-colors"
                    >
                      <Trash2 size={12} />
                      Delete Option
                    </button>
