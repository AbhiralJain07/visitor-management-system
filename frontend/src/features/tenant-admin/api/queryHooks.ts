...
// --- CUSTOM MASTER DATA HOOK — Backend connected, optimistic updates ---

const DEFAULT_MASTER_DATA: CustomMasterDataItem[] = [
...
export const useCustomMasterData = () => {
    ...
    queryFn: async () => {
      ...
      if (isOnline && serverAlive) {
        try {
          ...
          console.warn('Master data API failed, using default fallback:', err);
        }
      }
  ...
  const createItemMutation = useMutation({
    mutationFn: async (payload: Omit<CustomMasterDataItem, '_id' | 'createdAt'>) => {
      ...
      return { _id: `local-master-${Date.now()}`, ...payload, createdAt: new Date().toISOString() } as CustomMasterDataItem;
    },
    onMutate: async (payload) => {
      await queryClient.cancelQueries({ queryKey: ['custom-master-data'] });
      const previous = queryClient.getQueryData<CustomMasterDataItem[]>(['custom-master-data']);
      const optimistic: CustomMasterDataItem = { _id: `local-master-${Date.now()}`, ...payload, createdAt: new Date().toISOString() };
      queryClient.setQueryData<CustomMasterDataItem[]>(['custom-master-data'], old => [...(old || []), optimistic]);
      return { previous };
    },
    onError: (_err: any, _payload: any, context: any) => {
      if (context?.previous) queryClient.setQueryData(['custom-master-data'], context.previous);
    },
    onSettled: () => { queryClient.invalidateQueries({ queryKey: ['custom-master-data'] }); },
  });
  ...
  const updateItemMutation = useMutation({
    ...
    onMutate: async ({ id, payload }) => {
      await queryClient.cancelQueries({ queryKey: ['custom-master-data'] });
      const previous = queryClient.getQueryData<CustomMasterDataItem[]>(['custom-master-data']);
      queryClient.setQueryData<CustomMasterDataItem[]>(['custom-master-data'], old =>
